    ...
    public void receiveNewCertificat(MonAppliMessagerie.Certificat newCertif)
    {
        while(true)
        {
            if (!this._is_local())
            {
                ...
                try
                {
                    org.omg.CORBA.portable.OutputStream _output = this._request("receiveNewCertificat",true);
                    MonAppliMessagerie.CertificatHelper.write(_output,newCertif);
                    _input = this._invoke(_output);
                    return;
                }
                catch(org.omg.CORBA.portable.RemarshalException _exception)
            ...
            {
                org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke("receiveNewCertificat",_opsClass);
                if (_so == null)
                   continue;
                MonAppliMessagerie.PorteurOperations _self = (MonAppliMessagerie.PorteurOperations) _so.servant;
                try
                {
                    _self.receiveNewCertificat( newCertif);
                    return;
                }
                finally
                {
